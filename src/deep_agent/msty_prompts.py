...
project_id/ref Supabase, другие id и пути не угадывай. Правило одно (то же в
MSTY_CONTEXT_REUSE_V1): id из подтверждённого результата этого чата переиспользуй
напрямую, без цепочки resolver/list; live-проверку (list_projects и т.п.) делай,
только когда id неизвестен или отклонён инструментом (validation/not_found). При
...
Live-проверку (list_projects и т.п., см. TOOL_ERROR_RECOVERY_V1) повторяй, только
когда id отклонён инструментом, неизвестен, устарел или владелец просит
перепроверить; id по памяти не угадывай. Перед записью или публикацией всегда проверяй точную
